...
p1.setBackground('b')  # Set the background to white
p2 = win.addPlot(title="pitch", row=1, col=0)
p2.setBackground('w')  # Set the background to white
p3 = win.addPlot(title="altitude", row=2, col=0)
p3.setBackground('w')  # Set the background to white
...
# Initialize the low-pass filter for roll
roll_filter = LowPassFilter(alpha=0.3)  # You can adjust the alpha value to control the smoothing effect
        ...
        while True:
            if (datetime.now() > last_update + timedelta(milliseconds=update_interval * 1000)):
                ...
                if len(edge_coordinates) >= 2:
                    ...
                    # Calculate and filter roll angle
                    roll = calculate_pitch_and_roll(frame, horizon_line, center_vertical_line, center_horizontal_line)
                    roll_filter.update(roll)
                    filtered_roll = roll_filter.get_filtered_value()

                    # Display filtered roll value
                    cv2.putText(frame, f'Filtered Roll: {filtered_roll:.2f}', (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 2)
                    ...
                    p1.plot(x_axis_counters, roll_setpoint_history, pen=10)
